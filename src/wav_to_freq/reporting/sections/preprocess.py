...
def add_section_wav_specs(mdd: MarkdownDoc, context: PreprocessContext) -> None:
    ...
    duration_s = (
        (len(stereo.hammer) / float(stereo.fs))
        if float(stereo.fs) > 0
        else float("nan")
    )
    ...
    if stereo.autodetect is not None:
        ...
        rows.append(
            ["Autodetect score (right)", f"{stereo.autodetect.score_right:.6g}"]
        )
        rows.append(
            ["Autodetect confidence hi/lo", f"{stereo.autodetect.confidence_hi_lo:.3g}"]
        )
