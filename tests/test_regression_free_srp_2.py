...
def test_free_srp_2_hits_and_primary_frequencies() -> None:
    """Sanity test on a known multi-hit file.

    Expectations based on the archived report under:
    `examples/free_SRP_260119/output/free_srp_2/`
    """

    wav_path = Path("examples/free_SRP_260119/media/audio/free srp 2.wav")
    ...
    assert rep.n_hits_found == 11
    assert rep.n_hits_used == 11
    ...
    expected_primary_hz = 398.3642578125
    assert any(
    ...
    expected_peaks = [398.3642578125, 150.732421875, 344.53125]
    for expected in expected_peaks:
        assert any(
            abs(float(peak.fi_bin_hz) - expected) < 0.02 for peak in global_peaks
        ), f"Expected {expected:.2f} Hz in global peak list"
