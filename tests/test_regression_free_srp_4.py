...
def test_free_srp_4_hits_and_primary_frequency() -> None:
    """Sanity test on a known multi-hit file.

    Expectations based on the archived report under:
    `examples/free_SRP_260119/output/free_srp_4/`
    """

    wav_path = Path("examples/free_SRP_260119/media/audio/free srp 4.wav")
    ...
    assert rep.n_hits_found == 14
    assert rep.n_hits_used == 14
    ...
    expected_primary_hz = 333.7646484375
    assert any(
